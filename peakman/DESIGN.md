...
description: Барбершоп как вершина на топографическом листе — горизонтали сходятся к отметке «Герцена, 90А», сигнально-красная метка ведёт к записи.
colors:
  moss: "#232B21"
  moss-deep: "#1A2019"
  moss-up: "#2C3729"
...
только метка на карте (знак вершины) и действие «Записаться»; больше он нигде
...
A и M поднимаются пиками над кап-высотой, на первом пике стоит красная метка карты, рядом — костяная
табличка «УЛ. ГЕРЦЕНА, 90А» (moss на bone), как подпись объекта на карте. Это единственный логотип; текстовой версии нет.
...
### Signal
- `red` #E0392B — метка карты и кнопки записи; `red-deep` — hover.
- `ochre` #D9A441 — звёзды рейтинга и знак собаки: тёплые «отметки на карте»,
  не действие.

### Named Rules
- Красный — знак действия и метки. Не украшение, не рамка, не текст.
...
`legend-card` со знаком собаки 84–120px цвета `ochre`.

### Journal (журнал вершины)
Запись: строка label «дата · ИМЯ» → цитата lede в «ёлочках», дословно с
Яндекс Карт → звёзды `ochre` 15px. Две колонки на 720+, выравнивание по
верху. Под списком — источник и ссылка на все отзывы.

### Photo inset (вклейка)
Кадр с внутренней рамкой `sand-line`, подпись label «Вклейка 0N» + caption.

### Badge «можно с собакой»
В мета-строке hero: рамка 1.5px `sand`, знак собаки 24px `ochre`, текст 600.
Единственный бейдж на странице — под него отдан один факт.
...
(`0 -44 688 144`) для шапки. Fill — `bone`, метка — `red`, табличка адреса —
`bone` с текстом `moss` (Sofia 700, 29 ед.).

Моушн «восхождение» (один раз при загрузке, только под `.draw`): датум-линия
прочерчивается слева направо → буквы вырастают из неё по очереди
(`scaleY` от нижней кромки, `transform-box: fill-box`, шаг 90 мс, лёгкий
овершут) → метка падает на пик A → табличка выезжает → датум гаснет →
вокруг кончика метки тихо пульсирует красное кольцо (2.8 с, бесконечно).
`prefers-reduced-motion` выключает всё; без JS словомарка статична.
...
сдвигается JS ровно под кончик метки на словомарке — кольца всегда
...
внутренних колец `.rings` за адресом, центр — на красной метке.

### Altimeter (signature)
Шкала 0→90А по вертикали (1080+); бегунок-метка и красная линия
...
- Иконки — заливные силуэты из простых геометрических форм, один вес;
  собака, звезда, календарь, метка — одна семья.
